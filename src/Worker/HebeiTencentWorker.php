...
class HebeiTencentWorker extends BaseWorker implements Worker
{
    private $config;
    public  $pdo;
    ...
    {   
        try {
            ...
            if ($data_info['type'] == 'tencentDianbo') {
                ...
                if ($res == 200) {
                    $msg = 'dwk完成处理-tencentDianbo--河北展会-云点播存储-成功';
                } else {
                    $msg = 'dwk完成处理-tencentDianbo--河北展会-云点播存储-失败' . $res;
                }
            } else if ($data_info['type'] == 'tencentDianboUserVideo') {
                //云点播存储--商家视频管理
                $res = $this->actionTencentDianboUserVideo($data_info);
                if ($res == 200) {
                    $msg = 'dwk完成处理-tencentDianbo--河北商家-云点播存储-成功';
                } else {
                    $msg = 'dwk完成处理-tencentDianbo--河北商家-云点播存储-失败' . $res;
                }
            } else if ($data_info['type'] == 'tencentDeleteDianbo') {
                //云点播删除
                $res = $this->actionDeleteTencentDianbo($data_info);
                if ($res == 200) {
                    $msg = 'dwk完成处理-tencentDianbo--河北云点播删除-成功';
                } else {
                    $msg = 'dwk完成处理-tencentDianbo--河北云点播删除-失败' . $res;
                }
            } else if ($data_info['type'] == 'tencentUpdateDianbo') {
                //云点播修改 展会管理
                $res = $this->actionUpdateTencentDianbo($data_info);
                if ($res == 200) {
                    $msg = 'dwk完成处理-tencentDianbo--河北展会-云点播修改-成功';
                } else {
                    $msg = 'dwk完成处理-tencentDianbo--河北展会-云点播修改-失败' . $res;
                }    
            } else if  ($data_info['type'] == 'tencentUpdateDianboUserVideo') {
                //云点播修改 商家
                $res = $this->actionUpdateTencentDianboUserVideo($data_info);
                if ($res == 200) {
                    $msg = 'dwk完成处理-tencentDianbo--河北商家-云点播修改-成功';
                } else {
                    $msg = 'dwk完成处理-tencentDianbo--河北商家-云点播修改-失败' . $res;
                }    
            } else if ($data_info['type'] == 'tencentDianboYasuo') {
                //视频压缩 展会
                $res = $this->actionTencentDianboYasuo($data_info);
                if ($res == 200) {
                    $msg = 'dwk完成处理-tencentDianbo--河北展会-视频压缩-成功';
                } else {
                    $msg = 'dwk完成处理-tencentDianbo--河北展会-视频压缩-失败' . $res;
                }        
            } else {
                $msg  = '参数错误';
            }
