...
class ExhibitiontemplateSMSWorker extends BaseWorker implements Worker
{
    ...
    public function actionTaskDataSMSMeetSend($data_info = '') {
        ...
        $SendChinaInfo  =  self::SendChinaInfo($meetId, $userToId, $clientDomainName, $userMyId);//userToId中国企业
     ...
     * userToId
    */
    public function SendChinaInfo ($meetId,$userID, $clientDomainName, $userMyId = '') {
    ...
    public function actionTaskDataSMSMeetSendCancel($data_info = '') {
        ...
        $clientDomainName = ! empty( $data['clientDomainName'] ) ? $data['clientDomainName'] : self::hebeiurl;//展会模板的个人用户域名  
        self::ChinaSendCancelEmail($userToId,$exhibitorsId,$selectedDay,$deleteDate, $clientDomainName,$http_host_source, $userMyId);//中方发送取消
     ...
     * 中方发送取消邮件
     */
    public function ChinaSendCancelEmail($userID, $exhibitorsId = '', $selectedDay = '', $delete_date = '', $clientDomainName = '', $http_host_source = '', $userMyId = '') {
        $send_mail = $nameEn = '';
        $userInfo =  self::selectAppointmentInfo($userID, 3); //关联查出我的收到预约列表人数
