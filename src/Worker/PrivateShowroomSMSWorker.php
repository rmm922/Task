 ...
 * 邮件发送基类 私人企业
 */
class PrivateShowroomSMSWorker extends BaseWorker implements Worker
{
    private $config;
    public  $pdo;
    public  $redis;
    ...
    {   
        try {
            ...
            if ($data_info['type'] == 'taskDataSMSSendMeet') {//创建会议发起预约
                ...
                if ($res == 200) {
                    $msg = 'dwk完成处理-taskDataSMSAboutMeetSend--私人企业初次邮件短信发送-成功';
                } else {
                    $msg = 'dwk完成处理-taskDataSMSAboutMeetSend--私人企业初次邮件短信发送-失败' . $res;
                }
            } else if( $data_info['type'] == 'taskDataSMSSendMeetBegin' ) {//创建会议发起 配对成功 即将开始    
                //初始发送短信接口
                $res = $this->actionTaskDataSMSMeetBegin($data_info);
                if ($res == 200) {
                    $msg = 'dwk完成处理-taskDataSMSMeetBegin--私人企业即将开始邮件短信发送-成功';
                } else {
                    $msg = 'dwk完成处理-taskDataSMSMeetBegin--私人企业即将开始邮件短信发送-失败' . $res;
                }
            } else if( $data_info['type'] == 'taskDataSMSSendMeetCancel' ) {//创建会议发起 配对成功 取消会议    
                //初始发送短信接口
                $res = $this->actionTaskDataSMSMeetSendCancel($data_info);
                if ($res == 200) {
                    $msg = 'dwk完成处理-taskDataSMSSendMeetCancel--私人企业取消邮件和短信发送-成功';
                } else {
                    $msg = 'dwk完成处理-taskDataSMSSendMeetCancel--私人企业取消邮件和短信发送-失败' . $res;
                }
            } else {
                $msg  = '参数错误';
            }
    ...
    public function actionTaskDataSMSMeetSend($data_info = '') {
        ...
        $clientDomainName = ! empty( $data['clientDomainName'] ) ? $data['clientDomainName'] : self::hebeiurl;//私人企业的个人用户域名  
    ...
    */
    public function SendChinaInfo ($meetId,$userID, $clientDomainName) {
        ...
        if($userInfo) {
            $value = $userInfo[0];
            $sendUrl =  $clientDomainName.'index.php?app=shop/automaticLogin&flag=private&userId='.$userID.'&meetId='.$meetId.'&exhibitorsId='.$exhibitorsId;
    ...
    public function selectAppointmentInfo($ID = '', $type = '', $p46_notice_t_name = 'exhibiton_room') {
        ...
        if($type == 1) {
            ...
            $sql = 'SELECT ua.name__en as nameEn, ni.add_open_time, ni.add_stop_time,ni.exhibitors_id FROM p46_user_apply as ua JOIN p46_negotiation_info as ni ON ua.id = ni.exhibitors_id WHERE ni.id = ' . $ID;
        } else if($type == 5) {
    ...
    public function actionTaskDataSMSMeetBegin($data_info = '') {
        ...
        $clientDomainName    = ! empty( $data['client_domain_name'] ) ? $data['client_domain_name'] :  self::hebeiurl;//私人企业的个人用户域名  
