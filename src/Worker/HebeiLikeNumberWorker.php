...
class HebeiLikeNumberWorker extends BaseWorker implements Worker
{
    private $config;
    public  $pdo;
    private $params;
    ...
    {   
        try {
            ...
            if ($data_info['type'] == 'likeNumber') {
                ...
                if ($res == 200) {
                    $msg = 'dwk完成处理-likeNumber-河北点赞处理-成功';
                } else {
                    $msg = 'dwk完成处理-likeNumber--河北点赞处理-失败' . $res;
                }
            } else if ( $data_info['type'] == 'likeNumberMeet' ) {  
                //初始化原住民
                $res = $this->actionLikeNumberMeet($data_info);
                if ($res == 200) {
                    $msg = 'dwk完成处理-likeNumber-河北会议处理-成功';
                } else {
                    $msg = 'dwk完成处理-likeNumber-河北会议处理-失败' . $res;
                }  
            } else {
                $msg  = '参数错误';
            }
